...
| 11 | DevOps | nestjs-temporal-core's TEMPORAL_CONNECTION provider factory unconditionally calls NativeConnection.connect() (non-lazy, no timeout) even for non-worker registrations (backend calls getTemporalModule(false)) | Open, non-blocking | Found while diagnosing the 2026-09-16 backend boot hang (PR #8) — a real, verified oddity in the library's own code, not confirmed as the hang's root cause. Investigate whether backend actually needs this connection at all, and whether it can be disabled or bounded, independent of PR #8's mitigations |
